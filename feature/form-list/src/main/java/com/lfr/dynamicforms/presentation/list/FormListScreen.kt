            ...
            state.errorMessage != null -> {
                ...
                LazyColumn(
                    ...
                    if (state.pendingSubmissions.isNotEmpty()) {
                        item(key = "pending_section") {
                            PendingSubmissionsSection(
                                submissions = state.pendingSubmissions,
                                onRetry = onRetrySubmission,
                                onDiscard = onDiscardSubmission
                            )
                        }
                    }
                    items(state.forms, key = { it.formId }) { form ->
                        val hasDraft = form.formId in state.drafts
                        val pendingCount = state.pendingCountsByFormId[form.formId] ?: 0
                        val failedCount = state.pendingSubmissions.count {
                            it.formId == form.formId && it.status == SubmissionStatus.FAILED
                        }
                        Card(
                            modifier = Modifier
                                .fillMaxWidth()
                            ...
                            Column(modifier = Modifier.padding(16.dp)) {
                                Row(
                                    ...
                                    Icon(
                                        ...
                                        contentDescription = stringResource(R.string.open_form_content_description, form.title),
