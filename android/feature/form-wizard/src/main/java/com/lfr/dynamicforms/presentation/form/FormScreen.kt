        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            if (state.totalPages > 1) {
                ...
                val progressDescription = stringResource(
                    R.string.step_progress, state.currentPageIndex + 1, state.totalPages
                )
            ...
            when {
                state.isLoading -> {
                    FormSkeletonScreen(Modifier.fillMaxSize())
                }
                state.errorMessage != null -> {
                    ...
                    AnimatedContent(
                        ...
                        Column(
                            ...
                            val visibleElements = remember(state.currentPage, state.visibleElementIds) {
                                (state.currentPage?.elements ?: emptyList()).filter {
                                    state.visibleElementIds.isEmpty() || it.id in state.visibleElementIds
                                }
                            }
                            val sections = remember(visibleElements) {
                                groupElementsIntoSections(visibleElements)
                            }
