...
class MockInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val (code, json) = route(method, path, request)
        Timber.d("Mock: %s %s -> %d", method, path, code)
    ...
    private fun route(method: String, path: String, request: okhttp3.Request): Pair<Int, String> = when {
        ...
        method == "POST" && path.matches(Regex("/forms/[^/]+/submit")) -> {
            val idempotencyKey = request.header("Idempotency-Key")
            if (idempotencyKey != null) {
                processedKeys.add(idempotencyKey)
            }
            200 to SUBMIT_SUCCESS_JSON
        }
